...
class ByT5QGramGenerator:
    ...
    def generate_weight_file(self, training_texts: List[str], output_file: str, q: int = 2, max_samples: int = 500):
        """利用ByT5生成字符替换权重文件，优化训练速度"""
        print(f"开始生成权重文件: {output_file}")
        replacement_counts = defaultdict(int)  # 统计替换次数
        total_counts = defaultdict(int)  # 统计原始字符出现总次数

        # 限制训练样本数量以控制时间
        training_sample = training_texts[:max_samples]
        print(f"使用 {len(training_sample)} 个样本生成权重")

        # 对每个训练文本，生成变体并统计替换
        for i, text in enumerate(training_sample):
            if i % 50 == 0 and i > 0:
                print(f"已处理 {i}/{len(training_sample)} 个文本")
            ...
            # 对比原始文本与每个变体（限制变体数量）
            for variant in variants[:2]:  # 只使用前2个变体加速
...
def main():
    ...
    # 权重文件路径 - 明确标识这是使用测试集的20%生成的权重
    weight_file = os.path.join(result_dir, 'byt5_adapted_weights_from_test_20pct.csv')
    ...
    # 限制训练样本数量以控制时间
    max_training_samples = 800  # 可根据需要调整
    if len(training_texts) > max_training_samples:
        training_texts = random.sample(training_texts, max_training_samples)
    print(f"使用 {len(training_texts)} 个测试文本样本（20%部分）生成替换权重")
    ...
    def process_file(data: pd.DataFrame, source: str, is_test: bool = False) -> List[Dict[str, Any]]:
        ...
        # 测试模式下可以处理更少的记录以加速
        max_records = len(data) if not is_test else min(500, len(data))
        ...
        for idx in range(max_records):
            ...
            # 进度显示
            if (idx + 1) % max(1, max_records // 10) == 0:  # 每10%报告一次
