...
class BF:
    ...
    def create_AVa(self, original_bf, replaced_bf, original_text, replaced_text, record_id=""):
        """
        创建融合原始BF和替换后BF的AVa集合（带详细打印）
        original_bf: 原始文本的布隆过滤器
        replaced_bf: 替换后文本的布隆过滤器
        original_text: 原始文本
        replaced_text: 替换后文本
        record_id: 当前处理的记录ID（用于打印标识）
        """
        print(f"\n===== 开始生成记录 {record_id} 的AVa集合 =====")
        print(f"布隆过滤器长度: {self.bf_len}，原始文本: {original_text[:50]}...，替换后文本: {replaced_text[:50]}...")

        # 初始化AVa为全0列表（存储浮点数权重）
        AVa = [0.0] * self.bf_len
        print(f"初始化AVa集合: 长度 {len(AVa)}，初始值全为0.0")

        # 遍历布隆过滤器的每个位置，计算AVa的值
        for i in range(self.bf_len):
            # 打印进度（每50个位置打印一次，避免输出过多）
            if i % 50 == 0 and i > 0:
                print(f"已处理 {i}/{self.bf_len} 个位置...")

            # 获取原始BF和替换后BF在当前位置的值（0或1）
            orig_val = original_bf[i]
            repl_val = replaced_bf[i]
            print(f"\n位置 {i}/{self.bf_len}:")
            print(f"  original_bf[{i}] = {orig_val} (原始布隆过滤器值)")
            print(f"  replaced_bf[{i}] = {repl_val} (替换后布隆过滤器值)")

            # 情况1: 原始BF中为1 → AVa直接设为1.0
            if orig_val == 1:
                AVa[i] = 1.0
                print(f"  情况1: 原始BF为1 → AVa[{i}] = 1.0")

            # 情况2: 原始BF为0但替换后BF为1 → 计算替换权重
            elif orig_val == 0 and repl_val == 1:
                print(f"  情况2: 原始BF为0，替换后BF为1 → 计算替换权重...")
                # 计算当前位置的替换权重
                weight = self.get_replacement_weight(original_text, replaced_text, i)
                AVa[i] = weight
                print(f"  计算得到权重: {weight:.4f} → AVa[{i}] = {weight:.4f}")

            # 情况3: 两者都为0 → AVa保持0.0
            else:
                print(f"  情况3: 原始BF和替换后BF均为0 → AVa[{i}] = 0.0")

        print(f"\n===== 记录 {record_id} 的AVa集合生成完成 =====")
        # 打印AVa的统计信息（避免全量打印）
        non_zero_count = sum(1 for val in AVa if val > 0)
        avg_non_zero = sum(val for val in AVa if val > 0) / non_zero_count if non_zero_count > 0 else 0
        print(f"AVa集合统计: 非零值数量 {non_zero_count}/{self.bf_len}, 非零值平均值 {avg_non_zero:.4f}")
        return AVa

    def get_replacement_weight(self, original_text, replaced_text, position):
        """获取特定位置的替换权重（带打印）"""
        print(f"  计算位置 {position} 的替换权重:")
        # 反推可能的字符替换对
        char_pairs = self.find_char_pairs(original_text, replaced_text, position)
        print(f"  找到 {len(char_pairs)} 个可能的字符替换对: {char_pairs}")

        if not char_pairs:
            print(f"  未找到替换对，使用默认权重 {self.default_weight}")
            return self.default_weight

        # 计算所有替换对的权重平均值
        weight_sum = 0.0
        for idx, (orig_char, repl_char) in enumerate(char_pairs):
            key = f"{orig_char}-{repl_char}"
            weight = self.weights.get(key, self.default_weight)
            weight_sum += weight
            print(f"  替换对 {idx + 1}: {orig_char}→{repl_char}，权重 key={key} → {weight}")

        avg_weight = weight_sum / len(char_pairs)
        print(f"  替换对权重平均值: {avg_weight:.4f}")
        return avg_weight

    def find_char_pairs(self, original_text, replaced_text, position):
        """根据布隆过滤器位置反推可能的字符替换对（带打印）"""
        print(f"  反推位置 {position} 的字符替换对:")
        # 提取原始文本和替换文本的q-gram
        ...
        print(f"  原始文本中导致位置 {position} 的q-gram: {responsible_qgrams_original}")
        print(f"  替换文本中导致位置 {position} 的q-gram: {responsible_qgrams_replaced}")
...
def main():
        ...
        for idx, row in enumerate(data.itertuples()):
            try:
                ...
                if idx < 5:
                    ...
                    # 非前5条记录只打印简略信息
                    print(f"记录 {id_value} 的AVa集合生成完成（简略模式）")
    ...
    all_similarities = []  # 存储所有相似度值，用于计算平均值
    ...
    for i, rec1 in enumerate(file1_bloom_filters):
        for j, rec2 in enumerate(file2_bloom_filters):
            try:
                ...
                # 保存所有相似度值
                all_similarities.append(new_sim)
    ...
    if all_similarities:
        ...
        print(f"\n所有记录对的平均Dice相似度: {avg_similarity:.4f}")
        print(f"最小相似度: {min(all_similarities):.4f}, 最大相似度: {max(all_similarities):.4f}")
    else:
        print("\n未计算任何相似度值")
