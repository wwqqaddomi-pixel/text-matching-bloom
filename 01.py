...
def main():
    ...
    try:
        ...
        # 构建数据文件路径
        data1_path = os.path.join(script_dir, 'data', 'D:\实习\COMP1\COMP3850_PPRL\\ncvr_numrec_100_modrec_2_ocp_20_myp_0_nump_5.csv')
        data2_path = os.path.join(script_dir, 'data', 'D:\实习\COMP1\COMP3850_PPRL\\ncvr_numrec_100_modrec_2_ocp_20_myp_1_nump_5.csv')
    ...
    # 为每个记录计算布隆过滤器
    record_bloom_filters: List[Dict[str, Any]] = []
    ...
    # 创建相似度矩阵 (100x100)
    print("正在计算相似度矩阵...")
    matrix_size = min(100, len(record_bloom_filters))
    ...
    for i in range(matrix_size):
        for j in range(i + 1, matrix_size):
            sim = calc_bf_similarity(
                record_bloom_filters[i]['bloom_filter'],
                record_bloom_filters[j]['bloom_filter']
            )
    ...
    for i in range(matrix_size):
        for j in range(i + 1, matrix_size):
            if similarity_matrix[i, j] >= similarity_threshold:
                # 获取两个记录的详细信息
                rec1 = record_bloom_filters[i]
                rec2 = record_bloom_filters[j]
    ...
    try:
        ...
        if similar_pairs:
            ...
            pairs_path = os.path.join(result_dir, 'similar_records.xlsx')
